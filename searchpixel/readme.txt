...
Donate link: https://searchpixel.io
...
Search Pixel is a plugin that enhances your WordPress search by using AI-based semantic matching for your users to find relevant content.

== Description ==

Traditional WordPress search relies on basic keyword matching, which often overlooks the true semantic meaning of queries. Search Pixel revolutionizes this by leveraging an external API that converts your content into semantic embeddings. This ensures that when users search for something, they find truly relevant results, even if the exact keyword doesn’t appear in the post or page.

= How it works =

1. **Registration & Validation**  
   Upon activation, Search Pixel automatically registers your site with the SearchPixel Cloud API, securely generating an API key after validating your domain.

2. **Content Indexing**  
   The plugin sends your chosen posts and pages to the SearchPixel API, which splits them into sentence-like segments to generate embeddings. This allows for highly accurate content matching during searches.

3. **Search Handling**  
   When a user performs a search, the plugin transforms the search term into an embedding as well. It then queries the SearchPixel API for the closest matching embeddings, returning the most relevant (up to 20) results.
...
Yes, Search Pixel requires an active connection to the SearchPixel Cloud API to perform semantic indexing and searching. The SearchPixel API connection is done to api.searchpixel.io:8080 using TLS.
...
1. **SearchPixel Sync Settings** – A look at the simple admin settings page where you can configure and store your API key.

== Changelog ==

= 1.4.0 =
* Rebranded plugin from ThinkPixel to SearchPixel.
* Added new plugin entry points (searchpixel.php, uninstall, templates).
* Improved API error handling and response parsing.
* Made Error class implement JsonSerializable.
* Adjusted cron hook scheduling.
* Added type hints and performed general code cleanup.
* Sanitized UI inputs for better security.
* Improved API key polling behavior.
* Escaped titles in JavaScript to prevent XSS.
* Added new localization binaries and updated localization tooling.
...
* Search override to fetch curated results from SearchPixel.

== Upgrade Notice ==

= 1.0.0 =
First stable release. Securely sync content and override search with SearchPixel.
