jQuery(document).ready(async function ($) {
    ...
    function timeAgo(dateTimeStr) {
        ...
        if (diffMinutes < 5) {
            return searchpixelSettings.text.now;
        } else if (diffMinutes < 60) {
            return `${diffMinutes} ${searchpixelSettings.text.minutesAgo}`;
        } else if (diffMinutes < 160) {
            return searchpixelSettings.text.hourAgo;
        } else {
            const inputDateStr = inputDate.toISOString().split('T')[0];
            const nowDateStr = now.toISOString().split('T')[0];

            if (inputDateStr === nowDateStr) {
                return searchpixelSettings.text.today;
            } else {
                return dateTimeStr;
            }
        }
    ...
    const $apiStatus = $('#searchpixel-api-status');
    const $apiVersion = $('#searchpixel-api-version');

    // Function to check the API health
    async function checkAPIHealth() {
        try {
            const response = await $.ajax({
                url: searchpixelSettings.ping_url,
                type: 'POST',
                headers: {
                    'X-WP-Nonce': searchpixelSettings.wp_rest_nonce
                }
            });

            if (response.status === "ok" && response.version) {
                $apiStatus.text(searchpixelSettings.text.ok);
                $apiVersion.text(response.version);
            } else {
                $apiStatus.text(searchpixelSettings.text.error);
                $apiVersion.text('-');
            }
        } catch (error) {
            $apiStatus.text(searchpixelSettings.text.error);
    ...
    // Skip Search
    const $skipSearchButton = $('#searchpixel-skip-search-btn');
    const $skipSearchResetButton = $('#searchpixel-skip-reset-btn');
    const $skipSearchTextInput = $('#searchpixel-skip-search');
    const $skipSearchForm = $('#searchpixel-skip-form');
    const $skipSearchTableBody = $('#searchpixel-skip-results tbody');
    const $skipSearchPagination = $('#searchpixel-skip-pagination');
    const $skipIdsInput = $('#searchpixel-skip-ids-field');
    let skippedIdsCache = [];
    ...
    async function loadSkipSearch(query, offset) {
        try {
            const response = await $.ajax({
                url: searchpixelSettings.skip_search_url,
                method: 'POST',
                headers: {
                    'X-WP-Nonce': searchpixelSettings.wp_rest_nonce
                },
    ...
    $skipSearchButton.on('click', debounce(async function () {
        const query = $('#searchpixel-skip-search').val();
        if (query.length < 2) {
            alert(searchpixelSettings.text.minChars);
            return;
        }
        skippedIdsCache = [];
    ...
    // Check/uncheck all
    $('#searchpixel-skip-check-all').on('click', function () {
        const checked = $(this).prop('checked');
        $('#searchpixel-skip-results tbody input[type="checkbox"]').prop('checked', checked);
    });

    // Bulk Processing
    const $bulkIndexButton = $('#searchpixel-bulk-index-btn');
    const $progressBar = $('#searchpixel-bulk-progress-bar');
    const $statusMessage = $('#searchpixel-bulk-status-message');
    const $progressContainer = $('#searchpixel-bulk-progress-container');
    const $processedCountCell = $('#searchpixel-processed-count');
    const $remainingCountCell = $('#searchpixel-remaining-count');
    ...
    async function doBulkIndexing() {
        try {
            ...
            while (keepProcessing) {
                const response = await fetch(searchpixelSettings.bulk_post_processing_url, {
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(searchpixelSettings.text.bulkRequestError);
                }

                const data = await response.json();
                if (!data.success) {
                    throw new Error(searchpixelSettings.text.bulkResponseError);
                }
            ...
            $statusMessage.text(searchpixelSettings.text.bulkSuccess);
        } catch (error) {
            console.error(error);
            $statusMessage.text(searchpixelSettings.text.bulkError + error.message);
        } finally {
            // Re-enable button
