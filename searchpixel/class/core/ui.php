...
/**
 * UI Class. The SearchPixel plugin uses this for rendering the Settings page.
 *
...
class UI
{
    ...
    {
        add_options_page(
            __('SearchPixel Settings', Strings::Domain),
            __('SearchPixel', Strings::Domain),
            'manage_options',
    ...
    public function api_key_actions_handler()
    {
        if (
            ...
            if (isset($_POST['searchpixel_generate_api_key']) || isset($_POST['searchpixel_request_new_api_key'])) {
            ...
            } elseif (isset($_POST['searchpixel_regenerate_api_key'])) {
    ...
    public function skip_items_actions_handler()
    {
        if (
            ...
            if (isset($_POST['searchpixel_skip_selected_items'])) {
                // Handle skip/unskip selected pages
                $skip_ids = isset($_POST['skip_ids']) ? json_decode(sanitize_text_field(wp_unslash($_POST['skip_ids']))) : [];
                if (is_array($skip_ids) && ! empty($skip_ids)) {
                    $skip_ids = array_map('absint', $skip_ids);
                    $skip_ids = array_filter($skip_ids);
    ...
    {
        // Poll for the API key with a reasonable timeout (max 30 seconds).
