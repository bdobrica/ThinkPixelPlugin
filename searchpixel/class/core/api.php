...
/**
 * Api Class. The SearchPixel plugin uses this class for interacting with the SearchPixel API Gateway.
...
class Api
{
    ...
    /**
     * Registers the site with the SearchPixel API.
     *
     ...
     * This method is used to determine the maximum size of text that can be sent in a single batch to the SearchPixel API.
     * 
    ...
    /**
     * Processes raw posts data and sends it to the SearchPixel API.
    ...
    public function process_posts_raw(array $unprocessed_ids): array
    {
        ...
        if (isset($data['stored_ids']) && is_array($data['stored_ids']) && count($data['stored_ids']) > 0) {
            return $data['stored_ids']; // Return the stored IDs if available.
        }
    ...
    /**
     * Performs a search using the SearchPixel API.
     *
    ...
    /**
     * Refreshes the API key by calling the SearchPixel API.
     * 
    ...
    /**
     * Fetches a JWT token from the SearchPixel API.
     *
    ...
    /**
     * Pings the SearchPixel API to check its availability.
     *
