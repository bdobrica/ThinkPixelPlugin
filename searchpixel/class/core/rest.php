...
/**
 * Core of SearchPixel Plugin
 */

namespace SearchPixel\Core;

/**
 * Rest Class. The SearchPixel plugin uses this for REST API operations.
 *
 * @category Plugin
 * @package SearchPixel
 * @subpackage Core
...
class Rest
{
    ...
    private function register_validation_endpoint(): void
    {
        ...
        add_action('rest_api_init', function () {
            if ($this->get_api_key()) return;
            // Register the validation endpoint: ?rest_route=/searchpixel/v1/validate/
    ...
    public function handle_debug(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if ($jwt) {
            ...
            $report['get_jwt_details'] = $error ? $error->toArray() : null;
        }
        ...
        if (! $error) {
            ...
            $report['process_posts_raw_details'] = $error ? $error->toArray() : null;
        }
        ...
        if ($result) {
            ...
            $report['do_search_details'] = $error ? $error->toArray() : null;
        }
